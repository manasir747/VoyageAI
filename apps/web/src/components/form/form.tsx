...
import { Controller, FormProvider, type ControllerProps, type FieldPath, type FieldValues, type UseFormReturn, useForm, useFormContext } from "react-hook-form";
...
export function useZodForm<TSchema extends ZodTypeAny>({ schema, defaultValues, mode = "onTouched" }: { schema: TSchema; defaultValues?: Partial<React.ComponentProps<typeof useForm>>; mode?: Parameters<typeof useForm>[0]["mode"] }) {
  return useForm({
...
export const FormLabel = React.forwardRef<React.ElementRef<typeof LabelPrimitive.Root>, React.ComponentPropsWithoutRef<typeof LabelPrimitive.Root>>(({ className, ...props }, ref) => {
  return <LabelPrimitive.Root ref={ref} className={cn("text-sm font-medium leading-none text-foreground peer-disabled:cursor-not-allowed peer-disabled:opacity-70", className)} {...props} />;
});
...
export function FormControl({ children, ...props }: React.ComponentPropsWithoutRef<typeof Slot>) {
  ...
  return (
    <Slot
      ...
      aria-describedby={formItemContext ? `${formItemContext.id}-description ${formItemContext.id}-message` : undefined}
      {...props}
    >
      {children}
    </Slot>
...
export function FormDescription({ className, ...props }: React.HTMLAttributes<HTMLParagraphElement>) {
  const formItemContext = React.useContext(FormItemContext);
  return <p id={formItemContext ? `${formItemContext.id}-description` : undefined} className={cn("text-sm text-muted-foreground", className)} {...props} />;
}
...
export function FormMessage({ className, children, ...props }: React.HTMLAttributes<HTMLParagraphElement>) {
  ...
  return (
    <p id={formItemContext ? `${formItemContext.id}-message` : undefined} className={cn("text-sm font-medium text-error", className)} {...props}>
      {body}
    </p>
  );
}
...
export function FormErrorSummary({ className }: { className?: string }) {
  ...
  return (
    <div className={cn("rounded-2xl border border-error/20 bg-error/10 p-4", className)}>
      <div className="flex items-start gap-3">
        <CircleAlert className="mt-0.5 size-4 shrink-0 text-error" aria-hidden="true" />
        <div>
          <p className="font-medium text-foreground">Please fix the highlighted fields</p>
          <ul className="mt-2 space-y-1 text-sm text-muted-foreground">
            {errors.map((error, index) => (
              <li key={index}>{error?.message ?? "Invalid field"}</li>
            ))}
          </ul>
        </div>
      </div>
...
export function FormSubmit({ loading, children, className, ...props }: React.ComponentProps<typeof Button> & { loading?: boolean }) {
  return (
